...
def evaluate_model(
    *,
    ...
    features, labels, feature_names = read_file(dataset, use_dataframe=use_dataframe)
    ...
    if max_train_samples > 0 and len(y_train) > max_train_samples:
        print("subsampling training data from", len(X_train), "to", max_train_samples)
        sample_idx = np.random.choice(
            np.arange(len(X_train)), size=max_train_samples, replace=False
        )
    ...
    try:
        ...
        print("=" * 80)
        print(
            "WARNING: fitting timed out. If the program does not handle SIGALRM, it will be killed"
        )
        print("=" * 80)
    finally:
        signal.alarm(0)
    ...
    if cplx == -1 and (
        "complexity" in dir(algorithm) and algorithm.complexity is not None
    ):
    ...
    save_file = os.path.join(
        results_path, "_".join([dataset_name, est_name, str(random_state)])
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=0.0,
        type=float,
        help="Gaussian noise to addto the target",
    )
    parser.add_argument(
        "-feature_noise",
        action="store",
    ...
    parser.add_argument(
        "-sym_data", action="store_true", help="Use symbolic dataset settings"
    )
    parser.add_argument(
        "-skip_tuning",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=3600,
        type=int,
        help="Fit time limit (seconds) e.g. 3600 (1 hour). This is "
    ...
    algorithm = importlib.__import__(
        "methods." + args.ALG + ".regressor", globals(), locals(), ["*"]
    )
