...
def test_add_noise_modifies_y_keeps_x_and_expression():
    ev = _make_evaluated(100)
    noisy = add_noise([ev], level=0.1, rng=np.random.default_rng(3))[0]
...
def test_add_noise_zero_level_unchanged():
    ev = _make_evaluated(100)
    noisy = add_noise([ev], level=0.0, rng=np.random.default_rng(3))[0]
...
def test_add_noise_std_matches_level_times_rms():
    ...
    level = 0.15
    noisy = add_noise([ev], level=level, rng=np.random.default_rng(5))[0]
