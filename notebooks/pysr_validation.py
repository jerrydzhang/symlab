...
def _(
    ...
    for _i, _e in enumerate(entries):
        ...
        print(
            f"  {_i}: {_e.expression}  | y range [{_e.y.min():.2f}, {_e.y.max():.2f}]"
        )
    return (entries,)
...
def _(Expression, complexity, r2, results, sp):
    print(
        f"{'Idx':>3}  {'R2':>8}  {'Complex':>7}  {'Symbolic match':>15}  Ground truth / PySR"
    )
    print("-" * 100)
    ...
    for r in results:
        ...
        symbolic_match = (
            "EXACT"
            if diff == 0
            else ("CONST_DIFF" if diff.is_constant() else "DIFF")
        )
